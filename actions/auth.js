...
export const reroute = ({type, navigation, userLoading, isAuthenticated, user}) => async dispatch => {
  if (!userLoading) {
    if (type === 'intro') {
      if (isAuthenticated) {
        if (user.groups.includes('rider') || user.is_staff) {
          navigation.navigate("OrdersManager")
        } else {
